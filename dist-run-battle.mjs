...
// src/config/loader/game-config.ts
var game_config_exports = {};
__export(game_config_exports, {
  ConfigLoadError: () => ConfigLoadError,
  buildGameConfig: () => buildGameConfig,
...
function requiredModule(modules, id) {
  const value = modules[id];
  if (!value)
    throw new ConfigLoadError(`\u914D\u7F6E Manifest \u7F3A\u5C11\u5FC5\u9700\u6A21\u5757\uFF1A${id}`, id);
  return value;
}
function buildGameConfig(manifest, schemas, modules) {
  const ajv = new import_ajv.default({ allErrors: true, strict: true });
  ...
  const session = requiredModule(modules, "session");
  const inventory = requiredModule(modules, "inventory");
  const terror = requiredModule(modules, "terror");
  const supplies = requiredModule(modules, "supplies");
  const events = requiredModule(modules, "events");
  if (new Set(events.difficulties.map((difficulty) => difficulty.id)).size !== events.difficulties.length || events.difficulties.some(
    (difficulty) => events.dc + difficulty.dcOffset < 3 || events.dc + difficulty.dcOffset > 18
  ) || Math.abs(
  ...
  if (new Set(events.items.map((event) => event.id)).size !== events.items.length)
    throw new ConfigLoadError("events \u4E2D\u5B58\u5728\u91CD\u590D\u4E8B\u4EF6\u7C7B\u578B", "events");
  if (supplies.items.length < 3 || !supplies.items.some((item) => item.effect.type === "reroll_event_die"))
    throw new ConfigLoadError("\u4E8B\u4EF6\u9700\u8981\u81F3\u5C113\u79CD\u8865\u7ED9\u548C\u5355\u9AB0\u91CD\u6295\u9053\u5177", "events");
  const combatItems = requiredModule(
    modules,
    "combat_items"
  );
  const characters = requiredModule(modules, "characters");
  const economy = requiredModule(modules, "economy");
  const search = requiredModule(modules, "search");
  const combatRules = requiredModule(
    modules,
    "combat_rules"
  );
  const enemies = requiredModule(modules, "enemies");
  const prototypeRun = requiredModule(
    modules,
    "prototype_run"
  );
  ...
  for (const item of combatItems.items) {
    if (item.targetCharacterId && !characters.characters.some((c) => c.id === item.targetCharacterId))
      throw new ConfigLoadError(
  ...
  if (combatRules.dangerScaling.minimumDanger >= combatRules.dangerScaling.maximumDanger) {
  ...
  for (const [kind, rule] of Object.entries(combatRules.encounterRules)) {
    ...
    if (rule.requiredTier && !enemies.enemies.some((enemy) => enemy.tier === rule.requiredTier)) {
      throw new ConfigLoadError(`${kind} \u906D\u9047\u7F3A\u5C11\u5FC5\u9700\u9636\u7EA7\u654C\u4EBA`, "combat_rules");
    }
    if (!enemies.enemies.some((enemy) => rule.allowedTiers.includes(enemy.tier))) {
  ...
  const configuredAverage = search.rewardCountWeights.reduce(
    (total, weightedCount) => total + weightedCount.count * weightedCount.weight,
    0
  ) / rewardCountWeightTotal;
  if (Math.abs(configuredAverage - search.targetAverageRewardsPerPoint) > 1e-3) {
  ...
  if (mapLayout.minimumScale > mapLayout.defaultScale || mapLayout.defaultScale > mapLayout.maximumScale) {
    throw new ConfigLoadError(
      "prototype_run \u7684\u5730\u56FE\u9ED8\u8BA4\u7F29\u653E\u5FC5\u987B\u4F4D\u4E8E\u6700\u5C0F\u548C\u6700\u5927\u7F29\u653E\u4E4B\u95F4",
      "prototype_run"
    );
  }
  if (mapLayout.centerToRoadRatio.minimum > mapLayout.centerToRoadRatio.maximum) {
    throw new ConfigLoadError(
  ...
  for (const [kind, count] of Object.entries(prototypeRun.expectedRoomCounts)) {
    const pool = generation.roomNames[kind] ?? [];
  ...
  if (generation.startColumn >= grid.columns || generation.startRow >= grid.rows) {
    throw new ConfigLoadError("\u8D77\u59CB\u623F\u5750\u6807\u8D85\u51FA\u7F51\u683C\u8303\u56F4", "prototype_run");
  }
  if ((generation.startColumn === 0 || generation.startColumn === grid.columns - 1) && (generation.startRow === 0 || generation.startRow === grid.rows - 1)) {
    throw new ConfigLoadError(
  ...
  for (const [label, step, span] of [
    ...
    if (visibleRoadLength <= 0 || ratio < mapLayout.centerToRoadRatio.minimum - 1e-3 || ratio > mapLayout.centerToRoadRatio.maximum + 1e-3) {
      throw new ConfigLoadError(
  ...
  const dangerRampDistance = (prototypeRun.dangerRules.maximumDanger - 1) * prototypeRun.dangerRules.roomsPerLevel;
...
async function loadGameConfig(baseUrl = "/data") {
  const manifest = await fetchJson(`${baseUrl}/manifest.json`);
  const loaded = await Promise.all(
    manifest.modules.map(async (descriptor) => {
      const [schema, data] = await Promise.all([
        fetchJson(`${baseUrl}/${descriptor.schema}`),
        fetchJson(`${baseUrl}/${descriptor.path}`)
      ]);
...
// src/domain/combat/random.ts
var NON_ZERO_FALLBACK = 1831565813;
function seedToRandomState(seed) {
  if (typeof seed === "number") {
    const normalized2 = seed >>> 0;
    return normalized2 === 0 ? NON_ZERO_FALLBACK : normalized2;
  }
  let hash = 2166136261;
...
function randomInteger(state, minimum, maximum) {
  if (maximum < minimum) {
...
function weightedIndex(state, weights) {
...
// src/domain/combat/combat.ts
function isAlive(unit) {
  return unit.health > 0;
}
function stableActionOrder(left, right) {
  if (left.side !== right.side) return left.side === "hero" ? -1 : 1;
  return left.slot - right.slot || left.instanceId.localeCompare(right.instanceId);
}
...
function toUnit(input, combatRules) {
  return {
    ...
    ultimateDamageRatio: input.side === "hero" ? input.ultimateDamageRatio ?? 0 : 0,
    ultimateShield: input.side === "hero" ? input.ultimateShield ?? 0 : 0,
    ultimateShieldDurationMs: input.side === "hero" ? input.ultimateShieldDurationMs ?? 0 : 0,
    stunBonusMs: input.side === "hero" ? Math.min(2e3, input.stunBonusMs ?? 0) : 0,
    ...
    energy: input.side === "hero" ? Math.min(combatRules.maximumEnergy, Math.max(0, input.energy ?? 0)) : 0,
    ...
    aggroMultiplier: 1,
    damageReductionRatio: input.side === "hero" ? Math.min(0.9, Math.max(0, input.passiveDamageReductionRatio ?? 0)) : 0,
    passiveDamageReductionRatio: input.side === "hero" ? Math.min(0.9, Math.max(0, input.passiveDamageReductionRatio ?? 0)) : 0,
    damageBonusByEnemyTier: input.side === "hero" ? input.damageBonusByEnemyTier ?? {} : {},
    openingDamageRatio: input.side === "hero" ? Math.max(0, input.openingDamageRatio ?? 0) : 0,
    openingDamageUntilMs: input.side === "hero" ? Math.max(0, input.openingDamageDurationMs ?? 0) : 0,
    tauntUntilMs: 0,
    shield: 0,
    shieldUntilMs: 0,
    stunnedUntilMs: 0
...
function assertCombatants(combatants, combatRules) {
...
function createCombatState(seed, combatRules, combatants) {
  assertCombatants(combatants, combatRules);
  const units = combatants.map((combatant) => toUnit(combatant, combatRules)).sort(stableActionOrder);
  return {
    seed,
...
function selectWeightedAggroTarget(candidates, randomValue) {
  const living = candidates.filter((unit) => unit.side === "hero" && isAlive(unit)).sort(
    (left, right) => left.slot - right.slot || left.instanceId.localeCompare(right.instanceId)
  );
...
function selectFrontEnemy(units) {
  return units.filter((unit) => unit.side === "enemy" && isAlive(unit)).sort(
    (left, right) => left.frontlineOrder - right.frontlineOrder || left.slot - right.slot || left.instanceId.localeCompare(right.instanceId)
  )[0];
}
function selectHighestThreatEnemy(units, combatRules) {
  function threat(unit) {
    return unit.baseThreat + (unit.castingWarning ? combatRules.threatModifiers.castingWarningBonus : 0) + (unit.tier === "elite" ? combatRules.threatModifiers.eliteBonus : 0) + (unit.tier === "boss" ? combatRules.threatModifiers.bossBonus : 0);
  }
  return units.filter((unit) => unit.side === "enemy" && isAlive(unit)).sort(
    (left, right) => threat(right) - threat(left) || left.health / left.maxHealth - right.health / right.maxHealth || left.slot - right.slot || left.instanceId.localeCompare(right.instanceId)
  )[0];
}
function applyDamage(units, targetId, rawDamage, combatRules) {
...
function gainEnergy(units, unitId, requestedAmount, combatRules) {
  ...
  return {
    units: amount > 0 ? units.map(
      (candidate) => candidate.instanceId === unitId ? { ...candidate, energy: energyAfter } : candidate
    ) : units,
    amount,
    energyAfter
  };
}
function outgoingDamageMultiplier(attacker, target, atMs) {
  const tierRatio = target.tier ? attacker.damageBonusByEnemyTier[target.tier] ?? 0 : 0;
  const openingRatio = attacker.openingDamageUntilMs > atMs ? attacker.openingDamageRatio : 0;
  return 1 + tierRatio + openingRatio;
}
function expireTimedEffects(unit, elapsedMs) {
  let next = unit;
...
function fixedStep(state, combatRules) {
  ...
  let units = state.units.map((unit) => {
    ...
    return isAlive(activeUnit) ? {
      ...activeUnit,
      attackCooldownMs: activeUnit.attackCooldownMs - combatRules.fixedStepMs
    } : activeUnit;
  });
  const readyIds = units.filter(
    (unit) => isAlive(unit) && unit.attackCooldownMs <= 0 && unit.stunnedUntilMs === 0
  ).sort(stableActionOrder).map((unit) => unit.instanceId);
  const events = [];
  for (const attackerId of readyIds) {
    ...
    const variance = 1 + (varianceRoll.value * 2 - 1) * combatRules.damageVarianceRatio;
    ...
    const rawDamage = Math.max(
      combatRules.minimumDamage,
      Math.round(
        attacker.attack * (1 + attacker.basicDamageRatio) * outgoingDamageMultiplier(attacker, target, elapsedMs) * variance * (critical ? combatRules.criticalMultiplier : 1)
      )
    );
    ...
    if (target.side === "hero" && damageResult.targetHealthAfter > 0 && damageResult.damage > 0) {
      const charged = gainEnergy(
        units,
        target.instanceId,
...
function rejectUltimate(state, rejection) {
  return { accepted: false, rejection, state, events: [] };
}
function releaseUltimate(state, combatRules, heroInstanceId) {
  ...
  let units = state.units.map(
    (unit) => unit.instanceId === heroInstanceId ? { ...unit, energy: 0 } : unit
  );
  const events = [
    {
  ...
  const dealUltimateDamage = (targetId, baseDamage) => {
    const target = units.find((unit) => unit.instanceId === targetId);
    if (!target) return;
    const rawDamage = Math.max(
      combatRules.minimumDamage,
      Math.round(
        baseDamage * outgoingDamageMultiplier(hero, target, atMs) * (1 + hero.ultimateDamageRatio)
      )
    );
  ...
  switch (ultimate.type) {
    ...
    case "taunt_and_reduce_damage": {
      ...
      units = units.map(
        (unit) => unit.instanceId === heroInstanceId ? {
          ...unit,
          aggroMultiplier: ultimate.aggroMultiplier,
          damageReductionRatio: Math.min(
            0.9,
            1 - (1 - unit.passiveDamageReductionRatio) * (1 - ultimate.damageReductionRatio)
          ),
    ...
    case "all_enemy_damage_and_stun": {
      const targetIds = units.filter((unit) => unit.side === "enemy" && isAlive(unit)).sort(stableActionOrder).map((unit) => unit.instanceId);
      const rawDamage = Math.max(
        combatRules.minimumDamage,
      ...
      units = units.map(
        (unit) => unit.side === "enemy" && isAlive(unit) ? {
          ...unit,
          stunnedUntilMs: Math.max(unit.stunnedUntilMs, expiresAtMs)
        } : unit
      );
      for (const targetId of targetIds) {
        if (!units.some((unit) => unit.instanceId === targetId && isAlive(unit))) {
          continue;
        }
        events.push({
          type: "status-applied",
    ...
    case "team_shield": {
      const expiresAtMs = atMs + ultimate.durationMs;
      const targetIds = units.filter((unit) => unit.side === "hero" && isAlive(unit)).map((unit) => unit.instanceId);
      units = units.map(
        (unit) => targetIds.includes(unit.instanceId) ? {
          ...unit,
...
function advanceCombat(state, combatRules, deltaMs) {
  ...
  const maximumProcessable = combatRules.fixedStepMs * combatRules.maxCatchUpSteps;
...
// src/domain/combat/encounter.ts
function pickEnemy(state, candidates) {
  if (candidates.length === 0) {
    throw new Error("\u906D\u9047\u914D\u7F6E\u6CA1\u6709\u53EF\u7528\u7684\u654C\u4EBA");
  }
  const picked = weightedIndex(
    state,
    candidates.map((enemy2) => enemy2.spawnWeight)
  );
  const enemy = candidates[picked.index];
  if (!enemy) throw new Error("\u906D\u9047\u654C\u4EBA\u9009\u62E9\u8D8A\u754C");
  return { state: picked.state, enemy };
}
function referenceBudget(kind, danger, rules, definitions, nature) {
  const rule = rules.encounterRules[kind];
  const average = (pool2) => {
    const weight = pool2.reduce((sum, enemy) => sum + enemy.spawnWeight, 0);
    if (weight <= 0) throw new Error("\u906D\u9047\u9884\u7B97\u6CA1\u6709\u53EF\u7528\u7684\u654C\u4EBA");
    return {
      health: pool2.reduce((sum, e) => sum + e.maxHealth * e.spawnWeight, 0) / weight,
      dps: pool2.reduce(
        (sum, e) => sum + e.attack / e.attackIntervalMs * 1e3 * e.spawnWeight,
        0
      ) / weight
    };
  };
  ...
  const core = rule.requiredTier ? average(pool.filter((e) => e.tier === rule.requiredTier)) : { health: 0, dps: 0 };
  const escortCount = (rule.minimumUnits + rule.maximumUnits) / 2 - (rule.requiredTier ? 1 : 0);
  const offset = danger - rules.dangerScaling.minimumDanger;
  return {
    health: (core.health + escorts.health * escortCount) * rule.healthMultiplier * (1 + offset * rules.dangerScaling.healthPerLevel),
    dps: (core.dps + escorts.dps * escortCount) * rule.damageMultiplier * (1 + offset * rules.dangerScaling.damagePerLevel)
  };
}
function generateEncounter(seed, kind, danger, combatRules, enemiesConfig, nature = "physical") {
  ...
  const allowed = enemiesConfig.enemies.filter(
    (enemy) => rule.allowedTiers.includes(enemy.tier) && enemy.nature === nature
  );
  ...
  let healthMultiplier = (1 + levelOffset * combatRules.dangerScaling.healthPerLevel) * rule.healthMultiplier;
  let damageMultiplier = (1 + levelOffset * combatRules.dangerScaling.damagePerLevel) * rule.damageMultiplier;
  if (kind !== "boss") {
    ...
    const t = maximumDanger === minimumDanger ? 0 : (boundedDanger - minimumDanger) / (maximumDanger - minimumDanger);
    const blend = t * t * (3 - 2 * t);
    const healthBudget = near.health + (boss.health * curve.farHealthRatio - near.health) * blend;
    const dpsBudget = near.dps + (boss.dps * curve.farDamagePerSecondRatio - near.dps) * blend;
    healthMultiplier = healthBudget / selected.reduce((sum, e) => sum + e.maxHealth, 0);
    damageMultiplier = dpsBudget / selected.reduce(
      (sum, e) => sum + e.attack / e.attackIntervalMs * 1e3,
      0
    );
  }
...
// src/game/data.ts
function face(id, skin, hair2, extra) {
...
var HEROES = [
  { id: 7, nm: "\u94C1\u58C1\xB7\u77F3\u6562\u5F53", cls: "\u6B66", avatar: AVATARS[7], wu: 22, fa: 8, hp: 170, dr: 0.3, tank: true, ult: "\u94C1\u58C1\uFF1A\u9632\u5FA1\u578B\u6B66\u8005\uFF0C\u53D7\u5230\u7684\u4F24\u5BB3\u964D\u4F4E30%\uFF0C\u5929\u7136\u524D\u6392" },
  { id: 8, nm: "\u7384\u76FE\xB7\u9752\u7483", cls: "\u6CD5", avatar: AVATARS[8], wu: 8, fa: 22, hp: 172, dr: 0.28, tank: true, ult: "\u7384\u76FE\uFF1A\u9632\u5FA1\u578B\u6CD5\u5E08\uFF0C\u53D7\u5230\u7684\u4F24\u5BB3\u964D\u4F4E28%\uFF0C\u5929\u7136\u524D\u6392" },
  { id: 1, nm: "\u94C1\u638C\xB7\u5CB3\u6606", cls: "\u6B66", avatar: AVATARS[1], wu: 52, fa: 6, hp: 100, ult: "\u88C2\u77F3\u638C\uFF1A\u5BF9\u7269\u7406\u751F\u7269\u9020\u6210320%\u6B66\u529B\u4F24\u5BB3" },
  { id: 2, nm: "\u65AD\u5200\xB7\u621A\u98CE", cls: "\u6B66", avatar: AVATARS[2], wu: 48, fa: 5, hp: 96, ult: "\u65CB\u98CE\u65A9\uFF1A\u5168\u4F53\u8F93\u51FA\uFF0C\u7269\u7406\u623F\u5185\u4F24\u5BB3\u7FFB\u500D" },
  { id: 3, nm: "\u62F3\u5A18\xB7\u7EA2\u7F28", cls: "\u6B66", avatar: AVATARS[3], wu: 50, fa: 5, hp: 92, ult: "\u5D29\u62F3\uFF1A\u51FB\u6655\u7269\u7406\u751F\u72692\u56DE\u5408" },
  { id: 4, nm: "\u96F7\u6CD5\xB7\u95FB\u6F6E", cls: "\u6CD5", avatar: AVATARS[4], wu: 6, fa: 54, hp: 84, ult: "\u5929\u96F7\u5F15\uFF1A\u5BF9\u9B54\u6CD5\u751F\u7269\u9020\u6210320%\u6CD5\u529B\u4F24\u5BB3" },
  { id: 5, nm: "\u7B26\u5E08\xB7\u767D\u82B7", cls: "\u6CD5", avatar: AVATARS[5], wu: 5, fa: 46, hp: 88, ult: "\u7F1A\u7075\u7B26\uFF1A\u5168\u4F53\u8F93\u51FA\u5E76\u964D\u4F4E\u9B54\u6CD5\u751F\u7269\u653B\u901F" },
  { id: 6, nm: "\u5E7D\u706F\xB7\u8042\u665A", cls: "\u6CD5", avatar: AVATARS[6], wu: 5, fa: 50, hp: 80, ult: "\u5F15\u9B42\u706F\uFF1A\u5438\u6536\u9B54\u6CD5\u751F\u7269\u4F24\u5BB3\u4E3A\u5168\u961F\u62A4\u76FE" }
];
var ITEMS = [
...
// src/game/bridge.ts
var ATTACK_SCALE = 0.5;
var HEALTH_SCALE = 2.6;
var OFF_STAT_RATIO = 0.3;
var MISMATCH_RATIO = 0.4;
var BOSS_DANGER = 5;
var BOSS_HEALTH_FACTOR = 0.45;
var BOSS_ATTACK_FACTOR = 0.6;
var DEMO_COMBAT_DANGER = 3;
var BASE_INTERVAL = {
  tank: 1400,
  \u6B66: 900,
  \u6CD5: 1200
};
function gearTotals(hero, items, role) {
  const totals = {
    wu: 0,
    fa: 0,
    hpBonus: 0,
  ...
  for (const item of items) {
    const matched = item.type === "\u7269\u7406\u88C5" && hero.cls === "\u6B66" || item.type === "\u9B54\u6CD5\u88C5" && hero.cls === "\u6CD5" || item.type === "\u9632\u5177";
...
function heroCombatInput(hero, items, role, roomNature, slot) {
  const gear = gearTotals(hero, items, role);
  const wuTotal = hero.wu + gear.wu;
  const faTotal = hero.fa + gear.fa;
  const power = roomNature === "physical" ? wuTotal + faTotal * OFF_STAT_RATIO : faTotal + wuTotal * OFF_STAT_RATIO;
  const intervalKey = role === "tank" ? "tank" : hero.cls;
  const intervalMs = Math.max(
    300,
    Math.round(BASE_INTERVAL[intervalKey] * (1 - gear.attackSpeed))
  );
  const ultimate = role === "tank" ? {
    type: "taunt_and_reduce_damage",
    durationMs: 4e3,
    aggroMultiplier: 5,
    damageReductionRatio: 0.35
  } : hero.cls === "\u6B66" ? {
    type: "highest_threat_barrage",
    hitCount: 4,
    damageRatioPerHit: 0.9
  } : {
    type: "all_enemy_damage_and_stun",
    damageRatio: 1.8,
...
function buildRoomEnemies(seed, room, config) {
  const rules = config.combatRules;
  const enemiesConfig = config.enemies;
  if (room === "boss") {
    const nature2 = seedToNature(seed) === 0 ? "physical" : "ghost";
    const encounter2 = generateEncounter(
      seed,
      "boss",
      BOSS_DANGER,
      rules,
    ...
    return encounter2.enemies.map((enemy) => ({
      ...enemy,
...
function eventLine(event) {
  switch (event.type) {
    case "ultimate-released":
      return {
        atMs: event.atMs,
        text: `\u3010${(event.atMs / 1e3).toFixed(1)}s\u3011${heroName(event.unitId)} \u91CA\u653E\u5927\u62DB`,
        cls: "win"
      };
    case "unit-defeated":
      return {
        atMs: event.atMs,
        text: `\u3010${(event.atMs / 1e3).toFixed(1)}s\u3011${heroName(event.unitId)} \u88AB\u51FB\u5012`,
        cls: "lose"
      };
    default:
      return null;
...
var nameById = /* @__PURE__ */ new Map();
function heroName(instanceId) {
  return nameById.get(instanceId) ?? instanceId;
}
function runBattle(config, picks, equipped, room, seed = `demo:${Date.now()}`) {
  const rules = config.combatRules;
  const roomNature = room === "magic" ? "ghost" : "physical";
  nameById.clear();
  const heroes = [];
  ...
  lines.push({
    ...
    text: win ? `\u2605 \u6218\u6597\u80DC\u5229\uFF08${(state.elapsedMs / 1e3).toFixed(1)}\u79D2\uFF09\u2014 \u6218\u5229\u54C1\u5DF2\u5165\u80CC\u5305` : "\u2716 \u5168\u961F\u5012\u4E0B \u2014 \u7269\u8D44\u9057\u5931\uFF0C\u672C\u6B21\u63A2\u7D22\u5931\u8D25",
    cls: win ? "win" : "lose"
  });
  return {
    win,
    seconds: state.elapsedMs / 1e3,
    lines,
    heroHp: state.units.filter((unit) => unit.side === "hero").map((unit) => ({
      nm: heroName(unit.instanceId),
      hp: unit.health,
      maxHp: unit.maxHealth,
      role: unit.aggroWeight >= 3 ? "\u524D\u6392" : "\u8F93\u51FA"
    })),
    enemyHp: state.units.filter((unit) => unit.side === "enemy").map((unit) => ({
      nm: heroName(unit.instanceId),
      hp: unit.health,
      maxHp: unit.maxHealth
    }))
...
function itemsForHero(heroId, equipped) {
...
async function loadGameConfigFromFiles() {
  ...
  const { buildGameConfig: buildGameConfig2 } = await Promise.resolve().then(() => (init_game_config(), game_config_exports));
  return buildGameConfig2(manifest, schemas, modules);
}
void main();
